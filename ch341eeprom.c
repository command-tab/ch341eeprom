...
int main(int argc, char **argv)
{
    ...
    char *filename = NULL, *eepromname, operation = 0;
    ...
    while (TRUE) {
        int32_t optidx = 0;
        int8_t c = getopt_long(argc, argv, "hvdes:p:w:r:V:", longopts,
                               &optidx);
        if (c == -1)
            break;
    ...
    case 'V': // verify
        ...
        if (verify_failed)
            fprintf(stdout,
                    "Verification against file [%s] failed at offset [%d], EEPROM: %02hhX, file: %02hhX\n",
