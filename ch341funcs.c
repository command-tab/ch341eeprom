...
{
    uint8_t ch341outBuffer[EEPROM_READ_BULKOUT_BUF_SZ], *outptr = ch341outBuffer;
    int32_t actuallen = 0;
    ...
    *outptr = mCH341A_CMD_I2C_STM_END;
    ...
    int ret = libusb_bulk_transfer(devHandle, BULK_WRITE_ENDPOINT, ch341outBuffer, 3, &actuallen, DEFAULT_TIMEOUT);
...
size_t ch341ReadCmdMarshall(uint8_t *buffer, uint32_t addr, const struct EEPROM *eeprom_info)
{
    uint8_t *ptr = buffer;
...
int32_t ch341readEEPROM(struct libusb_device_handle *devHandle, uint8_t *buffer, uint32_t bytestoread, const struct EEPROM *eeprom_info)
{
    ...
    libusb_fill_bulk_transfer(xferBulkIn, devHandle, BULK_READ_ENDPOINT, ch341inBuffer, EEPROM_READ_BULKIN_BUF_SZ, cbBulkIn, NULL, DEFAULT_TIMEOUT);

    libusb_fill_bulk_transfer(xferBulkOut, devHandle, BULK_WRITE_ENDPOINT, ch341outBuffer, xfer_size, cbBulkOut, NULL, DEFAULT_TIMEOUT);
...
void cbBulkIn(struct libusb_transfer *transfer)
{
    ...
    case LIBUSB_TRANSFER_COMPLETED:
        ...
        for (unsigned i = 0; i < transfer->actual_length; ++i) {
            if (!(i % 16))
                fprintf(debugout, "\n   ");
...
int32_t ch341writeEEPROM(struct libusb_device_handle *devHandle, uint8_t *buffer, uint32_t bytesum, const struct EEPROM *eeprom_info)
{
    ...
    while (bytes) {
        outptr = i2cCmdBuffer;
        if (eeprom_info->addr_size > 1) {
            *outptr++ = 0xa0 | ((byteoffset >> 15) & (eeprom_info->i2c_addr_mask << 1));        // EEPROM device address
            *outptr++ = byteoffset >> 8 & 0xff; // MSB (big-endian) byte address
        } else {
            *outptr++ = 0xa0 | ((byteoffset >> 7) & (eeprom_info->i2c_addr_mask << 1)); // EEPROM device address
        }
        *outptr++ = byteoffset & 0xff;  // LSB of 16-bit    byte address
        ...
        while (page_size_left) {
            *outptr++ = mCH341A_CMD_I2C_STREAM;
            ...
            uint8_t to_write = (page_size_left < 29) ? page_size_left : (part_no != 0) + 28;
        ...
        ret = libusb_bulk_transfer(devHandle, BULK_WRITE_ENDPOINT, ch341outBuffer, payload_size, &actuallen, DEFAULT_TIMEOUT);
